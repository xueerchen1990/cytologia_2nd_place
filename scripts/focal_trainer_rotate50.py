...
import cv2
import pandas as pd
from utils import rotate_and_crop_no_padding, random_flip
...
def fix(x, m):
    return int(max(0, min(x, m)))

def fix_bbox(image, bbox):
    img_width, img_height = image.size
    x1, y1, x2, y2 = bbox
    x1 = fix(x1, img_width)
    y1 = fix(y1, img_height)
    x2 = fix(x2, img_width)
    y2 = fix(y2, img_height)
    return x1,y1,x2,y2
...
class BBoxClassificationDataset(ClassificationDataset):
    ...
    def __getitem__(self, i):
        ...
        if self.use_bbox_crop:
            ...
            img_name = f.split('/')[-1]
            if img_name in self.name_coord_dict:
                bbox = self.name_coord_dict[img_name]
                bbox = fix_bbox(im, bbox)
                if random.random() < 0.5:
                    # Apply draw_bbox_crop augmentation
                    base = 0.5+random.random()/2
                    scale = base+random.random()*(1-base)
                    im = random_crop_with_bbox(im, bbox, scale)
                elif random.random() < 0.5:
                    im = rotate_and_crop_no_padding(im, bbox)
            if random.random() < 0.5:
                im = random_flip(im)
            # plt.imshow(im)
            if im is None:
...
class CustomClassificationValidator(ClassificationValidator):
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        ...
        #self.dataloader = self.get_dataloader(args.data, 64)
